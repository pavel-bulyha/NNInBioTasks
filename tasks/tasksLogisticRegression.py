...
print('Таргеты тренировочные: ', y_train, 'Таргеты тестовые: ', y_test)
...
plt.show()
# Экспериментируя с моделью я понял - test_size оптимален 20 хотя и при меньших она не переобучается (оно и верно, модель такого типа обычно сложно переобучить), предел же итераций в данной библиотеке как будто ограничивает верхний, но не нижний край, как будто тут есть автоматическая реализация ранней остановки. Или модель просто слишком сложно переобучить